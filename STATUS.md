...
| 17 | **The night pass actually optimising anything** | `NightWorker`, `NightRun`, `VideoOptimiseStep` | 🔨 **the assembly now exists and is tested; it is still not bound.** `VideoOptimiseStep` joins probe, search, encode, verify and replace in the order the safety contract requires, with the refusals asserted against a counted `Replacer`. It cannot be wired yet: **`YuvSource` and `ProbeEncoder` have no implementation on any platform**, so the search has nothing to probe with and the VMAF gate has nothing to decode with. **Nothing has been compressed on your phone yet, and nothing will be until those two engines exist** |
| 17a | **`YuvSource` and `ProbeEncoder` on Android** | — | ⛔ **not built.** Two engine-api ports consumed by `ProbeAndSearch` and by `Verifier` and implemented by nothing, on either platform. Milestone 3's search and milestone 4's quality gate were both unit tested against fakes and neither has ever run on a device. They are the last thing between the encode path and a real optimise — and an emulator cannot prove them, because an ATD image has no hardware encoder |
