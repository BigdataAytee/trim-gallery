...
| 17 | **The night pass actually optimising anything** | `NightWorker`, `NightRun`, `VideoOptimiseStep` | 🔨 **everything it needs now exists; the last binding is deliberately still missing.** `VideoOptimiseStep` joins probe, search, encode, verify and replace in the order the safety contract requires, and both engines it was waiting for are built and bound ([#32](https://github.com/BigdataAytee/trim-gallery/pull/32), [#33](https://github.com/BigdataAytee/trim-gallery/pull/33)). What is left is binding `NightRun.Step` — the moment this app starts replacing your files unattended at 3am, which belongs in a change that does only that. **Nothing has been compressed on your phone yet** |
| 17a | **`YuvSource` and `ProbeEncoder` on Android** | [#32](https://github.com/BigdataAytee/trim-gallery/pull/32), [#33](https://github.com/BigdataAytee/trim-gallery/pull/33) | 🔨 **both built and bound.** `YuvSourceAndroid` decodes on a real runtime and six instrumented tests prove it on the emulator. `ProbeEncoderAndroid` encodes a window in hardware and decodes it back — and **that half cannot be proved in CI**, because an ATD image has no hardware encoder and BUILD.md rule 2 forbids the software one. What CI does assert is the rule: no hardware encoder means an empty window and a skipped file, never a software encode. The encode itself needs your phone |
